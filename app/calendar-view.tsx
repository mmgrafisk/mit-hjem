...
function EventModal({ initial, onClose, onDelete, onSave }: { initial: CalendarOccurrence | null; onClose: () => void; onDelete: (scope: "occurrence" | "series") => Promise<void>; onSave: (value: NewCalendarEvent, scope: "occurrence" | "series") => Promise<void> }) {
...
export function CalendarView({ householdId, userId, onSyncState }: CalendarViewProps) {
  ...
  const refresh = async () => { if (!householdId) return; const next = await loadCalendar(householdId); setEvents(next.events); setExceptions(next.exceptions); };
