...
const reminderOptions: Array<[CalendarReminderOffset | null, string]> = [
  [null, "Ingen"],
  [0, "Ved start"],
  [15, "15 minutter før"],
  [60, "1 time før"],
...
function EventModal({
  ...
  const [startsAt, setStartsAt] = useState(
    dateInputValue(initial?.occurrenceStart ?? defaultStart.toISOString()),
  );
  const [endsAt, setEndsAt] = useState(
    dateInputValue(initial?.occurrenceEnd ?? defaultEnd.toISOString()),
  );
  ...
  const [recurrenceEndOn, setRecurrenceEndOn] = useState(
    initial?.recurrenceEndOn ?? "",
  );
  const [reminderMinutes, setReminderMinutes] =
    useState<CalendarReminderOffset | null>(initial?.reminderMinutes ?? 60);
  ...
  const submit = async () => {
    if (!title.trim()) {
      setError("Skriv en titel til aftalen.");
      return;
    }
    if (new Date(endsAt) <= new Date(startsAt)) {
      setError("Sluttid skal ligge efter starttid.");
      return;
    }
    setSaving(true);
    setError("");
    try {
      await onSave(
        {
          ...
          recurrenceEndOn:
            recurrence === "once" ? null : recurrenceEndOn || null,
          reminderMinutes,
        },
        scope,
      );
      ...
      setError(
        reason instanceof Error ? reason.message : "Aftalen kunne ikke gemmes.",
      );
    } finally {
      setSaving(false);
    }
    ...
    <div className="modal-backdrop" onMouseDown={() => !saving && onClose()} role="presentation">
      ...
      >
        ...
        <header>
          ...
          <div>
            <h2 id="calendar-dialog-title">{initial ? "Redigér aftale" : "Ny aftale"}</h2>
            <p>Planlæg tid, sted, gentagelse og påmindelse.</p>
          </div>
        </header>
        <div className="calendar-form-grid">
          ...
          <label>
            <span>Slutdato</span>
            <input
              disabled={recurrence === "once" || scope === "occurrence"}
          ...
          <label>
            ...
            <select
              ...
              value={assignedTo}
            >
              <option value="">Hele husstanden</option>
            ...
            <select
              ...
              onChange={(e) =>
                setReminderMinutes(
                  e.target.value === "none"
                    ? null
                    : (Number(e.target.value) as CalendarReminderOffset),
                )
              }
          ...
          {initial?.recurrence !== "once" ? (
            <fieldset className="field-span-2 scope-choice">
...
export function CalendarView({
  ...
  const [editing, setEditing] = useState<CalendarOccurrence | null | undefined>(
    undefined,
  );
  const range = useMemo(
    () =>
      mode === "month"
        ...
        : {
            ...
            end: addDays(startOfWeek(focusDate), 6),
          },
    [focusDate, mode],
  );
  const days = useMemo(() => {
  ...
  const refresh = async () => {
    ...
    const next = await loadCalendar(householdId);
  ...
  useEffect(() => {
    const task = window.setTimeout(
      () => void refresh().catch(() => onSyncState?.("error")),
      0,
    );
  ...
  const save = async (
    ...
    if (!householdId || !userId)
      throw new Error("Log ind for at gemme aftalen.");
    onSyncState?.("saving");
    if (editing && scope === "occurrence" && editing.recurrence !== "once")
      await saveCalendarOccurrenceException(householdId, userId, editing, {
        title: value.title,
    ...
    else await saveCalendarEvent(householdId, userId, value, editing?.id);
    await refresh();
    onSyncState?.("synced");
  };
  const remove = async (scope: "occurrence" | "series") => {
    if (!editing || !householdId || !userId) return;
    onSyncState?.("saving");
    if (scope === "occurrence" && editing.recurrence !== "once")
      await saveCalendarOccurrenceException(householdId, userId, editing, {
        cancelled: true,
      });
    else await deleteCalendarEvent(householdId, editing.id);
    await refresh();
  ...
  return (
    <div className="calendar-page">
      <div className="module-toolbar">
        <div>
          <h2>Fælles kalender</h2>
          <p>Måneder, uger og gentagne aftaler samlet ét sted.</p>
        </div>
        <button
          className="primary-button"
      ...
      <div className="calendar-controls">
        ...
        <div className="calendar-period">
          <button aria-label="Forrige" onClick={() => move(-1)} type="button">
            <ChevronLeft size={17} />
          </button>
          <strong>
            {mode === "month"
              ? monthFormatter.format(focusDate)
              : `${dayFormatter.format(range.start)} – ${dayFormatter.format(range.end)}`}
          </strong>
          <button aria-label="Næste" onClick={() => move(1)} type="button">
      ...
      <section className={`calendar-grid ${mode}`}>
        <header>
        ...
        <div>
          {days.map((day) => {
            const dayOccurrences = occurrences.filter(
              (item) =>
                new Date(item.occurrenceStart).toDateString() ===
                day.toDateString(),
            );
            const isOutside =
              mode === "month" && day.getMonth() !== focusDate.getMonth();
            return (
              <article
                className={isOutside ? "outside" : ""}
                key={day.toISOString()}
              >
                <b>{day.getDate()}</b>
                <div>
      ...
      <section className="calendar-agenda">
        <h3>Kommende i visningen</h3>
        {occurrences.length ? (
          occurrences.map((item) => (
            <button
              key={`agenda-${item.id}-${item.occurrenceKey}`}
              onClick={() => setEditing(item)}
              type="button"
            >
              <time>{dayFormatter.format(new Date(item.occurrenceStart))}</time>
              <span>
                <strong>{item.title}</strong>
                <small>
                  <Clock3 size={13} />
                  {item.allDay
                    ? "Hele dagen"
                    : timeFormatter.format(new Date(item.occurrenceStart))}
                  {item.location ? (
                    <>
                      <MapPin size={13} />
                      {item.location}
